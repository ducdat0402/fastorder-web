...
import { getAdminFoods, getCategories, createFood, updateFood, deleteFood } from '../services/api';
...
const Admin = () => {
  ...
  // Cập nhật trạng thái is_available ngay khi tick
  const handleAvailableToggle = async (food) => {
    try {
      await updateFood(food.id, { ...food, is_available: !food.is_available });
                  ...
                  <tr key={food.id} className="border-b">
                    ...
                    <td className="p-2">
                      <input
                        ...
                        onChange={() => handleAvailableToggle(food)}
                        className="h-5 w-5"
                      />
                    </td>
